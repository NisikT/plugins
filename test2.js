(function () {
    ...
    // Ensure both plugins are not initialized multiple times
    if (window.plugin_interface_cleaned_menu && window.plugin_new_interface_logo_ready) return;

    // Initialize both plugin flags
    window.plugin_interface_cleaned_menu = true;
    window.plugin_new_interface_logo_ready = true;

    // ==============================
    // ❌ REMOVED (from the first script):
    ...
    // ==============================
    // ✅ KEEP: CHILD MODE API HOOK (from the first script)
    // ==============================
    ...
    // ==============================
    // ✅ KEEP: PRELOAD SYSTEM (from the first script)
    // ==============================
    var globalInfoCache = {};
    ...
    // ==============================
    // ✅ KEEP: RATING COLORS (from the first script)
    // ==============================
    function getColor(v) {
        if (v <= 3) return "red";
    ...
    // ==============================
    // ✅ NEW INTERFACE LOGIC (from the second script)
    // ==============================
    function startNewInterface() {
        if (!Lampa.Maker || !Lampa.Maker.map || !Lampa.Utils) return;
        
        addStyles();
        applyGlobalCardFix(); // Adds global card fix for new interface
        ...
        Lampa.Settings.listener.follow("open", function (event) {
            if (event.name === "main") {
                if (Lampa.Settings.main().render().find('[data-component="style_interface"]').length === 0) {
                    Lampa.SettingsApi.addComponent({ component: "style_interface", name: "Стильний інтерфейс" });
                }
