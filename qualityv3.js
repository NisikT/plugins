(function () {
    ...
    if (details.length) {
        ...
        Lampa.Parser.get({ search: e.data.movie.title || e.data.movie.name, movie: e.data.movie, page: 1 }, function(response) {
            if (response && response.Results) {
                ...
                var badges = [];
                // Горизонтальне розташування іконок
  ...
  var style = '<style>\
    ...
    .card-quality-badges { position: absolute; top: 0.3em; right: 0.3em; display: flex; flex-direction: row; flex-wrap: nowrap; gap: 0.2em; pointer-events: none; z-index: 5; }\
